...
def inspect_art_sports_plan_risk(items: list[dict]) -> dict:
    ...
    if count['冲'] > count['保']:
        warnings.append('冲刺志愿比例偏高，保底志愿不足，存在滑档风险。')
    if count['保'] < 8:
        warnings.append('保底志愿数量不足，建议增加2025最低综合分更低的院校专业。')
    if no_adjustment_count:
        warnings.append('存在未服从调剂志愿，达到投档线后仍可能因专业未录取而退档。')
    if not items:
        warnings.append('当前暂无志愿方案。请先完善艺体档案并点击「智能生成」；需管理员在后台启用大模型。')

    level = '高' if high_risk_count or len(warnings) > 1 else '中' if warnings else '低'
    if not warnings:
        warnings.append('当前方案中冲稳保结构相对合理，仍需以官方填报系统为准（艺体无官方综合分位次）。')
...
def inspect_plan_risk(items: list[dict]) -> dict:
    if any(item.get('art_sports_mode') or item.get('artSportsMode') for item in items):
        return inspect_art_sports_plan_risk(items)
    count = {'冲': 0, '稳': 0, '保': 0, '垫': 0}
    warnings = []
    high_risk_count = 0
    no_adjustment_count = 0
