...
def _letterbox_portrait_canvas(image: Image.Image, width: int = POSTER_WIDTH, height: int = POSTER_HEIGHT) -> Image.Image:
    """等比缩放后留白居中，适用于宽高比不一致的模板。"""
    source = image.convert('RGBA')
    contained = ImageOps.contain(source, (width, height), Image.Resampling.LANCZOS)
    canvas = Image.new('RGBA', (width, height), (245, 247, 251, 255))
...
def load_poster_background(template_path: Path | None = None) -> Image.Image:
    ...
    with Image.open(path) as image:
        source = image.convert('RGBA')
    if source.size == (POSTER_WIDTH, POSTER_HEIGHT):
        return source.copy()
    ...
    if abs(source_ratio - target_ratio) < 0.02:
        return source.resize((POSTER_WIDTH, POSTER_HEIGHT), Image.Resampling.LANCZOS)
    if source_ratio > target_ratio * 1.08:
        return _letterbox_portrait_canvas(source)
    return _fit_portrait_canvas(source)
