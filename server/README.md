...
1. **AppID 必须一致**：微信开发者工具 → 详情 → 基本信息，填写正式 AppID（如 `wx58ed9703d22d85c2`），不能是 `touristappid`。域名是在该 AppID 对应的小程序后台配置的。
2. **真机预览 vs 真机调试**：预览模式强制校验域名；若调试模式能用、预览不能用，删除手机上该小程序后重新扫码，或预览时右上角 ··· → **开发调试** → **打开调试**。
3. **看具体错误**：我的 → **网络连接检测**，把弹窗里的 `错误：request:fail ...` 完整内容发出来排查。
4. **院校库页**：若首页/院校库能加载数据，说明网络已通，档案页失败多半是业务报错（会显示「档案保存失败」而非「无法连接后端」）。
